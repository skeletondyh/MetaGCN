...
from metagcn.models import SampleAndAggregate, SAGEInfo, Node2VecModel
from metagcn.minibatch import EdgeMinibatchIterator, SimpleMinibatchIterator
from metagcn.neigh_samplers import UniformNeighborSampler
from metagcn.utils import load_data
...
def train(train_data, test_data=None):
    ...
    context_pairs = train_data[-1] if FLAGS.random_context else None
    ...
    author_v_author_adj_info_ph = tf.placeholder(tf.int32, shape=author_v_author_adj.shape)
    author_v_author_adj_info = tf.Variable(author_v_author_adj_info_ph, trainable=False, name='author_v_author_adj_info')

    author_p_author_features_ph = tf.placeholder(tf.float32, shape=author_p_author_features.shape)
    author_p_author_features_info = tf.Variable(tf.float32, trainable=False, name='author_p_author_feats')
    ...
    # Init variables
    sess.run(tf.global_variables_initializer(), feed_dict={author_p_author_adj_info_ph:author_p_author_adj,
                                                            author_v_author_adj_info_ph:author_v_author_adj,
                                                            author_p_author_features_ph:author_p_author_features,
                                                            author_v_author_features_ph:author_v_author_features})
